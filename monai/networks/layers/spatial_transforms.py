...
class AffineTransform(nn.Module):
    def __init__(
        ...
        In this case `theta` is equivalent of (ndim+1, ndim+1) input ``matrix`` of ``scipy.ndimage.affine_transform``,
        ...
        if tuple(self.theta.shape[1:]) not in ((3, 3), (4, 4)):
            raise ValueError(f"affine must be Nx3x3 or Nx4x4, got: {self.theta.shape}")
    ...
    def forward(self, src):
        ...
        # broadcast the batch dim of theta, to match `src`
